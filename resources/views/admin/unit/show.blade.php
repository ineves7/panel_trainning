...
@section('title', 'Dados da Unidade')
    ...
    <div class="col-md-4 col-12">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title">Dados da Unidade</h4>
        </div>
        <div class="card-body">
          <form class="form form-horizontal" method="POST" action="{{ route('unidades.update', $unit_selected->id) }}">
            @csrf()
            @method('PUT')
            <div class="row">
              <div class="col-12">
                <div class="mb-1 row">
                  ...
                  <div class="col-sm-9">
                    <input type="text" value="{{ $unit_selected->name }}" id="name" class="form-control" name="name" placeholder="Nome do Unidade" />
                  </div>
                </div>
              </div>
              <div class="col-12">
                <div class="mb-1 row">
                  ...
                  <div class="col-sm-9">
                    <input type="text" value="{{ $unit_selected->sigla }}" id="sigla" class="form-control" name="sigla" placeholder="sigla" />
                  </div>
                </div>
              </div>
              <div class="col-12">
                  ...
                  <div class="col-sm-9">
                    <select class="select2 form-select" id="city_id" name="city_id">
                      @foreach($cities as $city)
                          <option value="{{ $city->id }}" {{ $city->id === $unit_selected->city_id ? 'selected' : '' }}>{{ $city->state->uf }} - {{ $city->name }}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
              ...
              <div class="col-sm-9 offset-sm-3">
                ...
                <form method="POST" name="form-delete" action="{{ route('unidades.destroy', $unit_selected->id) }}">
                    @csrf()
                    @method('delete')
                    <button type="submit" class="btn btn-danger" style="position: relative; float: left;" 
                      onclick="return confirm('Tem certeza que deseja deletar a unidade?');">Deletar
                    </button>
                </form>
              </div>
            </div>
