<!-- BEGIN: Footer-->
<footer class="footer footer-light {{($configData['footerType'] === 'footer-hidden') ? 'd-none':''}} {{$configData['footerType']}}">
  <p class="clearfix mb-0">
    <span class="float-md-start d-block d-md-inline-block mt-25">COPYRIGHT &copy;
      <script>document.write(new Date().getFullYear())</script><a class="ms-25" href="https://www.arraial.rj.gov.br/" target="_blank"> - Prefeitura Municipal de Arraial do Cabo</a>,
      <span class="d-none d-sm-inline-block">Todos os direitos reservados.</span>
    </span>
    <span class="float-md-end d-none d-md-block">Desenvolvido pelo CODE</span>
  </p>
</footer>
<button class="btn btn-primary btn-icon scroll-top" type="button"><i data-feather="arrow-up"></i></button>
<!-- END: Footer-->
